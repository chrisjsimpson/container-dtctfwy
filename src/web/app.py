...
def upload_complete():
    ...
    <a href="{ url_for('translate') }">Start Transcribing and Translating</a> 
    """
...
def background_transcribe(app=None, filename=None):
    ...
    subprocess.run(
        f"whisper /usr/src/app/uploads/{filename} --model medium", shell=True
    )
...
def background_translate(app=None, filename=None):
    ...
    subprocess.run(
        f'subprocess.run(f"whisper uploads/{filename} --model medium --language German --task translate")'
    )
...
def translate():
    ...
    filename = session.get("file")["filename"]
    background_translate(filename=filename)
    return """<h1>Translation &amp; Transcription started! We will send you your transcription/translation is complete</h1>
            <p>Enter your email address to be contacted:</p>
            <form action='/alert' method='GET'>
              Email: <input type='email' name='email' /><br />
              <input type='submit' />
